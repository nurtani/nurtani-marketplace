...
const totalSustainabilityPoints = computed(() => {
  return cartItems.value.reduce((total, item) => {
    return total + (item.product?.points || 0) * item.quantity;
  }, 0);
});
...
<template>
  <div class="min-h-screen bg-[#F5F7F5] py-8 px-4 sm:px-6 flex justify-center">
        ...
        >
          ...
          <div>
            ...
            <p class="text-sm text-gray-200 leading-snug">
              Produk langsung dari petani lokal yang telah terverifikasi NurTani
            </p>
          </div>
        </div>
      </div>
